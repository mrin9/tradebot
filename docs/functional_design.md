...
3. **Entry**
   ...
   - Position size (lots):
     - Uses realized PnL + initial budget if compounding (`invest_mode="compound"`).
     - Or uses initial budget if fixed.
...
- `position_config`: budget, SL/TSL, targets, pyramiding, instrument type, `python_strategy_path`, etc.
- Optional services:
