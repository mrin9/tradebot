...
- **`--sl-points`**:
  - Absolute points off premium (e.g., 15 points on option price).
- **`--target-points`**:
  - Comma‑separated profit targets. Can be used with pyramiding logic.
- **`--tsl-points`**:
  - Point‑based trailing stop loss increment.
- **`--tsl-id`**:
