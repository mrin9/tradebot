...
class ContractManager:
    ...
    def refresh_active_contracts(self, date_range_keyword: str = "today"):
        ...
        input_coll = db[settings.NIFTY_CANDLE_COLLECTION]
    ...
    def _identify_contracts(self, db, spot_price, dt: datetime):
        ...
        opt_ref = master.find_one({
            "exchangeSegment": "NSEFO", "name": "NIFTY", "series": "OPTIDX",
            "contractExpiration": {"$gte": dt_str}
        }, sort=[("contractExpiration", 1)])
        ...
        if opt_ref:
            ...
            for opt in options:
                ...
                # Determine Moneyness
                if strike == atm_strike: moneyness = "ATM"
                elif is_call: moneyness = "ITM" if strike < spot_price else "OTM"
                else: moneyness = "ITM" if strike > spot_price else "OTM" # Put
