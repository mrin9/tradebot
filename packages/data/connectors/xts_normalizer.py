...
class XTSNormalizer:
    ...
    def parse_xts_master_data(content: str) -> List[Dict]:
        ...
        if not content: 
            return []
        return [
            item for item in (XTSNormalizer.parse_xts_master_line(l) for l in content.strip().split('\n')) 
            if item is not None
        ]
    ...
    def normalize_1501_tick_event(data: Dict) -> Dict:
        ...
        bid = bid_info.get('Price') if isinstance(bid_info, dict) else (str(data.get('bi', '')).split('|')[1] if '|' in str(data.get('bi')) else None)
        ask_info = data.get('AskInfo')
        ask = ask_info.get('Price') if isinstance(ask_info, dict) else (str(data.get('ai', '')).split('|')[1] if '|' in str(data.get('ai')) else None)
