...
class TradePersistence:
    ...
    def sync_live_cycle(self, session_id: str, pos: Position):
        ...
        try:
            ...
            update = {"$set": {"tradeCycles.$": cycle_data}, "$set": {"updatedAt": datetime.now(DateUtils.MARKET_TZ).replace(microsecond=0).isoformat()}}
    ...
    def save_session_summary(self, session_id: str, trades: List[Position], config: Dict, daily_pnl: Dict, is_live: bool = True):
        """
        ...
        try:
            ...
            for cid, chunks in cycle_groups.items():
                ...
                for c in chunks:
                    if str(c.status).upper().startswith("TARGET"):
                        ...
                        targets.append({
                            "step": c.achieved_targets or (int(str(c.status).split('_')[1]) if '_' in str(c.status) else 1),
    ...
    def update_session_status(self, session_id: str, status: str, is_live: bool = True):
        ...
        self.db[col].update_one(
            {"sessionId": session_id},
            {"$set": {"status": status, "updatedAt": datetime.now(DateUtils.MARKET_TZ).replace(microsecond=0).isoformat()}}
        )
