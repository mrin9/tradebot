    ...
    def build_config_summary(fund_manager: Any, mode: str = "live") -> dict:
        ...
        if isinstance(fund_manager, dict):
            ...
            if "budget" not in config:
                config["budget"] = 0
            if "invest_mode" not in config:
                config["invest_mode"] = "fixed"
            if "sl_points" not in config:
                config["sl_points"] = 0
            if "target_points" not in config:
                config["target_points"] = []
            if "tsl_points" not in config:
                config["tsl_points"] = 0
            if "use_be" not in config:
                config["use_be"] = False
            return config
        ...
        config.update(
            {
                ...
                "python_strategy_path": fund_manager.position_config.get("python_strategy_path")
