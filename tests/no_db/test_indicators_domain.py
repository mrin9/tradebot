...
from packages.tradeflow.types import InstrumentCategoryType, MarketIntentType
...
def test_canonical_indicator_names():
    ...
    for i in range(100):
        calc.add_candle(
            ...
            instrument_category=InstrumentCategoryType.SPOT, instrument_id=26000
        )
...
def test_fund_manager_indicator_mapping_cache(mock_strategy_path):
    ...
    config = {
        "symbol": "NIFTY", "timeframe_seconds": 60,
        "indicators": [{"id": "SMA_20", "type": "SMA", "params": {"period": 20}}],
        "pythonStrategyPath": mock_strategy_path,
        "position_config": {
            "budget": 100000, "invest_mode": "FIXED_BUDGET", "sl_points": 20, "target_points": [40],
            "tsl_points": 0, "use_be": True, "instrument_type": "OPTIONS", "strike_selection": "ATM",
            "price_source": "close", "symbol": "NIFTY", "pyramid_steps": [100], "pyramid_confirm_pts": 10
        }
    }
