...
def reset_xts_manager():
    """Reset XTSManager singletons before each test to prevent mock leakage."""
...
def test_market_login_success(mock_xts):
    ...
    XTSManager._market_client = None # Force reset
...
def test_get_ohlc_parsing(mock_xts):
    ...
    response = XTSManager._get_market_client().get_ohlc(
        ...
        startTime='Mar 04 2026 100000', endTime='Mar 04 2026 110000', compressionValue=60
    )
...
def test_get_quote_parsing(mock_xts):
    ...
    response = XTSManager._get_market_client().get_quote(
        ...
        xtsMessageCode=1501, publishFormat="1"
    )
    assert response['type'] == 'success'
...
def test_live_get_quote_structure():
    ...
    response = XTSManager.call_api(
        ...
        xts_message_code=1501, publish_format="1"
    )
    assert response['type'] == 'success'
...
def test_live_get_ohlc_structure():
    ...
    response = XTSManager.call_api(
        "market", "get_ohlc",
        exchange_segment=1, exchange_instrument_id=nifty_id,
        start_time='Feb 27 2026 100000', end_time='Feb 27 2026 110000', compression_value=60
    )
    assert response['type'] == 'success'
