...
def add_rsi(df: pl.DataFrame, col: str, period: int = 14) -> pl.DataFrame:
    df = df.with_columns(
        (pl.col(col) - pl.col(col).shift(1)).alias("delta")
    )
    df = df.with_columns([
...
def verify_golden_trades(fixture_path):
    ...
    if not cursor:
        print(f"❌ Nifty data not found in collection '{NIFTY_COL}'. Ensure `seed_test_data.py` was run.")
        sys.exit(1)
        
    df_nifty = pl.DataFrame(cursor)
    
    df_nifty = df_nifty.with_columns(
        pl.from_epoch(pl.col("t"), time_unit="s").alias("datetime")
    )
    
    # 2. Resample exactly to 3-minutes (Triple Lock Timeframe)
    res_nifty = df_nifty.group_by_dynamic(
        "datetime",
        every="3m",
        closed="left",
        label="left"
