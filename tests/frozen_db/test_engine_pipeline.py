...
def run_strategy_backtest(db, strategy_id, pos_overrides=None, use_real_strategy=True, start_time=None):
    ...
    pos_config = {
        ...
        "invest_mode": "fixed",
        "sl_points": 100.0, # High SL to let strategy exit
        "target_points": "500,1000",
        "tsl_points": 0.0,
        "use_be": False,
        "symbol": "NIFTY",
...
def test_ema_triple_lock(db_conn):
    ...
    fm, signals, counts = run_strategy_backtest(
        ...
        pos_overrides={"invest_mode": "compound", "sl_points": 15.0, "target_points": "15,25,50"}
    )
...
def test_ema_cross_rsi(db_conn):
    ...
    """
    fm, signals, counts = run_strategy_backtest(db_conn, "ema-cross-rsi-180s")
...
def test_supertrend_price_active(db_conn):
    ...
    """
    fm, signals, counts = run_strategy_backtest(db_conn, "st-price-300s-active")
...
def test_macd_dual_real(db_conn):
    ...
    """
    fm, signals, counts = run_strategy_backtest(db_conn, "macd-180s-dual")
...
def test_position_manager_parameters(db_conn):
    ...
    fm, signals, counts = run_strategy_backtest(db_conn, "ema-cross-rsi-180s", pos_overrides)
