...
@pytest.fixture
def mock_xts():
    with patch("packages.data.connectors.xts_wrapper.XTSManager.call_api") as m_api, \
         patch("packages.data.connectors.xts_wrapper.XTSManager.get_market_data_socket") as ms:
...
def test_live_market_subscription(mock_xts):
    ...
    args, kwargs = m_api.call_args_list[0]
    ...
    args, kwargs = m_api.call_args_list[-1]
