...
async def get_ticks(
    ...
    limit: int = 1000,
    start_dt: Optional[str] = Query(None, alias="start-dt"),
    end_dt: Optional[str] = Query(None, alias="end-dt"),
    ...
    resampled = df.sort("t").group_by_dynamic("t", every=period).agg([
    ...
    return {
        "ticks": result_ticks,
        "hasMoreOld": has_more_old,
        "hasMoreNew": has_more_new
    }
