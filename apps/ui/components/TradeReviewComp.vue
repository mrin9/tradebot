    ...
    <div class="review-charts-wrapper">
        <template v-if="backtestStore.selectedBacktest">
            <div class="charts-container">
                <ReviewChartComp :instrumentId="currentTrade?.instrumentId" :timeframe="candleTimeframe"
                    :indicators="indicators" :markers="markers" :levels="levels"
                    :symbol="currentTrade?.instrumentDesc || 'Contract'" :maxTimestamp="maxTimestamp" />
            </div>
        </template>
        ...
        <div v-else class="empty-state">
            <div class="empty-content">
                ...
                <Button label="Refresh Backtests" icon="pi pi-refresh" @click="backtestStore.fetchBacktests()"
                    class="mt-3" />
            </div>
        </div>
    </div>
